...
```text
<你的路徑>/learnboard/ 或 <你的路徑>/learnmart/
├── manage.py
├── pyproject.toml
├── config/
└── board/              # LearnBoard；LearnMart 對應為 marketplace/
```
...
- 專案 A 需要 Django 4.2
- LearnBoard 與 LearnMart 都需要 Django 5.2
...
## 1-8 讀懂兩個 `pyproject.toml` 的差異

**目前專案節錄｜兩份 `pyproject.toml` 的 dependencies**

```toml
[project]
name = "learnmart"
requires-python = ">=3.13"
learnboard: django>=5.2,<5.3
learnmart:  django>=5.2,<5.3  ＋ pillow>=11.0
```

- `[project]`：一個 TOML table
- `name = "..."`：key 與字串 value
- `dependencies = [...]`：TOML 字串陣列
- LearnBoard 沒有圖片上傳，所以不需要 Pillow
- LearnMart 的 `ImageField` 需要 Pillow；這是兩個專案在環境層的主要差異
...
- Django 是兩個專案都需要的 dependency；Pillow 只在 LearnMart 是執行需要的 dependency
- Coverage 放在 `dev` 群組，主要供開發與檢查
- `package = false` 表示這兩個 repository 都當作應用專案，不把專案自身建成可發布 Python package
